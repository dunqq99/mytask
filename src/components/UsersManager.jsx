...
function AdminTeamsManager({ token, API_BASE_URL, users }) {
  ...
  const fetchTeamRoles = async (targetOwnerId) => {
    try {
      const url = targetOwnerId 
        ? `${API_BASE_URL}/api/team-roles?ownerId=${targetOwnerId}` 
        : `${API_BASE_URL}/api/team-roles`;
